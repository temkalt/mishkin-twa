...
### Переменные фронта (`.env` в корне)

| Переменная | Зачем |
|---|---|
| `VITE_API_URL` | адрес API, по умолчанию `/api` (в dev проксируется Vite) |
| `VITE_BOT_USERNAME`, `VITE_APP_SHORT_NAME` | из них собирается ссылка «Поделиться товаром» (`t.me/<bot>/<app>?startapp=product_<id>`). Не заданы — приложение делится веб-адресом карточки; подставлять чужой юзернейм по умолчанию нельзя, такая ссылка уводит покупателя в посторонний магазин |
| `VITE_CHANNEL_URL` | канал в попапе подписки (по умолчанию `t.me/mishkin_candles`) |

Переменные `VITE_*` попадают в собранный бандл и видны всем — секретов там быть
не должно.
...
`POST /api/payments/create` умеет два способа подтверждения:

| Ответ | Когда | Что делает приложение |
|---|---|---|
| `confirmationToken` | по умолчанию (реальный магазин) | рисует форму ЮKassa внутри Mini App — Telegram пользователь не покидает |
| `confirmationUrl` | `{ mode: 'redirect' }` и всегда в режиме эмулятора | открывает страницу оплаты во внешнем браузере (`WebApp.openLink`) |

Форму показывает виджет `checkout-widget.js`; `return_url` ему намеренно не
передаётся — с ним ЮKassa перестаёт вызывать обработчики `success`/`fail` и
уводит пользователя из Telegram. Если виджет не поднялся (скрипт не
загрузился, токен истёк), приложение само повторяет запрос с
`{ mode: 'redirect' }`, поэтому заказ не зависает. Событие виджета
доказательством оплаты не считается: статус всё равно подтверждается опросом
`GET /api/payments/status/:orderId`.

Единственная точка запуска платежа на фронте — страница `/order/:id`
(`src/pages/OrderResult.tsx`). Чекаут только создаёт заказ и уводит на неё с
`state.autoPay`.

## Склад (остатки)

У товара есть поле «Остаток, шт.» (`Product.stock`):

- **пусто (`null`)** — учёт не ведётся, товар доступен всегда. Так работают все
  товары, созданные до появления склада: включать учёт можно по одному;
- **число** — сколько штук можно продать. Списывается **в той же транзакции, что
  создание заказа** (`UPDATE … WHERE stock >= qty`), поэтому два одновременных
  заказа не уведут остаток в минус;
- **0** — покупатель видит «нет в наличии»: кнопка «в корзину» гаснет, оформить
  нельзя. Товар при этом остаётся в каталоге (в отличие от «Скрыть», которое
  снимает его с продажи через `inStock = false`).

Остаток резервируется в момент оформления, а не оплаты — иначе последнюю свечу
успеют оплатить двое. Обратно на склад он возвращается, когда заказ переводят в
статус **«Отменён»**; повторная отмена ничего не задваивает (`Order.stockReturnedAt`).
Неоплаченный онлайн-заказ держит резерв до отмены — если покупатель пропал,
заказ нужно отменить в админке.

Покупателю остаток показывается, только когда это осмысленно: «осталось N шт.»
при N ≤ 5, «нет в наличии» при нуле. Корзина не даёт увеличить количество выше
остатка и не пускает в чекаут, пока в ней лежит недоступный товар.
...
npm run test:e2e          # заказ → оплата эмулятором → статус PAID, склад и права доступа
```
...
npx prisma migrate resolve --applied 20260818021500_product_stock
```
...
(несколько фото — по ссылке в строке; остаток на складе — пусто, если учёт не
нужен), промокоды, рассылка по пользователям бота, пост в канал с кнопкой в
Mini App.
